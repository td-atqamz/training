...
class FaceMesh:
    ...
    def isOpen(self, image, face_mesh_results, face_part, threshold=5):
        ...
        status = {}
        if face_part == "MOUTH":
            INDEXES = self.mpFaceMesh.detect_mouth_area(
                face_mesh_results, image_height, image_width, threshold
            )
        elif face_part == "LEFT EYE":
            INDEXES = self.mpFaceMesh.detect_left_eye_area(
                face_mesh_results, image_height, image_width, threshold
            )
        elif face_part == "RIGHT EYE":
            INDEXES = self.mpFaceMesh.detect_right_eye_area(
                face_mesh_results, image_height, image_width, threshold
            )
        else:
            return
        ...
        for face_no, face_landmarks in enumerate(
            ...
            cv2.putText(
                output_image,
                f"FACE {face_no+1}{face_part} {status[face_no]}.",
                (10, image_height - 40),
    ...
    def masking(self, image, filter_img, face_landmarks, face_part, INDEXES):
        ...
        try:
            ...
            _, face_part_height, landmarks = self.getSize(
            ...
            if face_part == "MOUTH":
                ...
                location = (
                    int(center[0] - filter_img_width / 2),
