...
def check_win():
    if (
        any(np.sum(board, 1) == 3)
        or any(np.sum(board, 0) == 3)
        or sum(np.diag(board)) == 3
        or sum(np.diag(board[::-1])) == 3
    ):
        return True
    if (
        any(np.sum(board, 1) == -3)
...
while move > 0:  # loop yang akan terus berjalan selama jumlah move masih lebih dari 0
    print(board)
    play_turn()
    if check_win():
